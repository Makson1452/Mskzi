...
int main()
{
    int short rlz1 = 1023; //Начальное состояние первой рекурентной линии задержки
    ...
    long long int rlz2[2] = { 13523568565465,54266675758343421 }; //Начальное состоянии второй рекурентной линии задержки
    long long int gamma = 0; //Выходная последовательность
    for (int i = 0; i < 64; i++) {
        gamma = gamma << 1; //Сдвиг выходной последовательности на 1 бит
        bit_gamma = ((((rlz1 >> 10) ^ (rlz1 >> 7)) & 0x1) ^ (((rlz2[0] >> 97) ^ (rlz2[0] >> 91)) & 0x1)); //Формирование символа выходной последовательности в определенном такте
        gamma = gamma + bit_gamma; //Прибавление символа выходной последовательности
        rlz1 = rlz1 << 1; //Сдвиг РЛЗ1 на 1 бит влево
        rlz1 = rlz1 + (((rlz1 >> 10) ^ (rlz1 >> 7)) & 0x1); // Запись бита обратной связи в РЛЗ1
        ...
        rlz2[1] = rlz2[1] + (((rlz2[0] >> 97) ^ (rlz2[0] >> 91)) & 0x1); //Запись бита обратной связи в РЛЗ2
    }
